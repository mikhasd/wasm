...
Graph graph_new(i32 edges){
  ...
  graph->edges = calloc(edges * edges, sizeof(i8));
  return graph;
}
...
void graph_add_edge(Graph graph, i32 from, i32 to, i8 weight){
  graph->edges[calc_offset(graph, from, to)] = weight;
  graph->edges[calc_offset(graph, to, from)] = weight;
}

i8 graph_get_weight(Graph graph, i32 from, i32 to){
...
List get_neighbors(Graph this, i32 vertex){
  ...
  for(i32 i = 0; i< this->edges_size; i ++){
    if(vertex != i){
      i8 weight = graph_get_weight(this, i, vertex);
      if(weight > 0){
...
i32* calculate_cost_table(Graph this, i32 from){
  i8* costs = malloc(this->edges_size * sizeof(i8));
